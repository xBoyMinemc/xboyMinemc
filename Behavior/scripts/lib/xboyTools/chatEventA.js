...
if(M.indexOf("..check a ") != -1){
        ...
        msg.targets = []
        let playerX;
        getPlayers(overworld).forEach(player => {if(player.nameTag == M.replace("..check a ","")){playerX = player}});
        let i = 0;
        for(;i < 36;i++){
        try{
...
if(M.indexOf("..check z ") != -1){
        ...
        msg.targets = []
        let playerX;
        getPlayers(overworld).forEach(player => {if(player.nameTag == M.replace("..check z ","")){playerX = player}})
        let i = 0;
        for(;i < 36;i++){ try{
if(playerX.getComponent("inventory").container.getItem(i) != undefined){
